...
func (ctrl *ChatInviteCtrl) NewChatInvite(ctx *gin.Context) {
	var (
		params = new(dto_chat_invite.NewChatInviteReq)
		uid    int64
		resp   *xhttp.Resp
		err    error
	)
	...
	resp = ctrl.chatInviteService.NewChatInviteReq(params, uid)
	if resp.Code > 0 {
