DROP TABLE IF EXISTS `users`;
CREATE TABLE `users` (
  `uid` bigint NOT NULL COMMENT '用户ID 系统生成',
  `lark_id` varchar(32) NOT NULL DEFAULT '' COMMENT '账户ID 用户设置',
  `password` varchar(32) NOT NULL DEFAULT '' COMMENT '密码',
  `udid` varchar(40) NOT NULL DEFAULT '' COMMENT '注册设备唯一标识',
  `status` tinyint(1) NOT NULL DEFAULT '0' COMMENT '用户状态',
  `nickname` varchar(64) NOT NULL DEFAULT '' COMMENT '昵称',
  `firstname` varchar(32) NOT NULL DEFAULT '' COMMENT 'firstname',
  `lastname` varchar(32) NOT NULL DEFAULT '' COMMENT 'lastname',
  `gender` tinyint(1) NOT NULL DEFAULT '0' COMMENT '性别',
  `birth_ts` bigint NOT NULL DEFAULT '0' COMMENT '生日',
  `email` varchar(64) NOT NULL DEFAULT '' COMMENT 'Email',
  `mobile` varchar(32) NOT NULL DEFAULT '' COMMENT '手机号',
  `reg_platform` tinyint(1) NOT NULL DEFAULT '0' COMMENT '注册平台',
  `platform` tinyint(1) NOT NULL DEFAULT '0' COMMENT '最后登录的平台',
  `server_id` int NOT NULL DEFAULT '0' COMMENT '分配的ws服务器',
  `city_id` int NOT NULL DEFAULT '0' COMMENT '城市ID',
  `created_ts` bigint NOT NULL DEFAULT '0',
  `updated_ts` bigint NOT NULL DEFAULT '0',
  `deleted_ts` bigint NOT NULL DEFAULT '0',
  PRIMARY KEY (`uid`),
  UNIQUE KEY `uid` (`uid`),
  UNIQUE KEY `lark_id` (`lark_id`),
  KEY `idx_deletedTs` (`deleted_ts`),
  KEY `idx_larkId` (`lark_id`),
  KEY `idx_status` (`status`),
  KEY `idx_udid` (`udid`),
  KEY `idx_mobile` (`mobile`),
  KEY `idx_platform` (`platform`),
  KEY `idx_gender` (`gender`),
  KEY `id_gender_cityId` (`gender`,`city_id`),
  KEY `id_larkId_password` (`lark_id`,`password`),
  KEY `id_mobile_password` (`mobile`,`password`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;